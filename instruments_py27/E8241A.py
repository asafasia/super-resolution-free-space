...
class E8241A_MG(Instrument):
    """"A class for controlling Agilent E8241A using GPIB"""
    ...
    def setup_MG(self, freq=None, power=None, set_on=True):
        ...
        if freq is not None:
            self.MG.write('FREQ %fe6' % freq)
            sleep(0.100)
        if power is not None:
            self.MG.write('POW:AMPL %f' % power)
            sleep(0.100)
        if set_on:
            self.MG.write("OUTP 1")
            sleep(0.100)
    ...
    def update_property(self, property, value):
        ...
        if property not in self.update_functions:
            raise Exception("E8241A_MG.update_property: Property is not supported")
