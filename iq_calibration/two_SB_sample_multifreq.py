"""Generate positive and negative sidebands for multiple frequencies. Sample using OPX"""
...
ampl = 0.01
pulse_length = 22000
neg_amp_factor1 = 1.0 #for negative sideband
pos_amp_factor1 = 0.0 #for positive sideband
neg_amp_factor2 = 1.0 #for negative sideband, 2nd IF freq.
pos_amp_factor2 = 0.0 #for positive sideband, 2nd IF freq.
phase2 = 0.0 #relative
phase3 = 0.0 #relative
phase4 = 0.0 #relative
...
cg.add_mixer("mixer1",{(lo_freq, if_freq1):[1.0,0.0,0.0,1.0],(lo_freq, if_freq2):[1.0,0.0,0.0,1.0]})
cg.add_mixer("mixer2",{(lo_freq, if_freq1):[1.0,0.0,0.0,1.0],(lo_freq, if_freq2):[1.0,0.0,0.0,1.0]})
cg.add_mixed_input_element("SB1",lo_freq+if_freq1,lo_freq,I_channel,Q_channel,"mixer1")
cg.add_mixed_input_element("SB3",lo_freq+if_freq2,lo_freq,I_channel,Q_channel,"mixer1")
cg.add_mixed_input_element("SB4",lo_freq+if_freq2,lo_freq,I_channel,Q_channel,"mixer2")
...
cg.add_operation("SB3", "control_const", "const_pulse")
cg.add_operation("SB4", "control_const", "const_pulse")
cg.add_mixed_readout_element("SB2",lo_freq+if_freq1,lo_freq,I_channel,Q_channel,{"out_I":2, "out_Q":1},"mixer2",200)
...
with program() as measurement_prog:
    ...
    z_rotation(phase2, "SB2")
    z_rotation(phase3, "SB3")
    z_rotation(phase4, "SB4")

    with for_(rep, 0, rep < 1, rep + 1):
    # with infinite_loop_():
        play("control_const"*amp(neg_amp_factor1), "SB1")
        play("control_const" * amp(neg_amp_factor2), "SB3")
        play("control_const" * amp(pos_amp_factor2), "SB4")
        measure("readout"*amp(pos_amp_factor1), "SB2", "samples",
...
qm.set_mixer_correction("mixer1", int(if_freq1), int(lo_freq), tuple(model_corr_mat(g,phi).flatten())) #negative sideband, IF freq. 1
qm.set_mixer_correction("mixer1", int(if_freq2), int(lo_freq), tuple(model_corr_mat(g,phi).flatten())) #negative sideband, IF freq. 2
m_pos = model_corr_mat(g,phi)@(np.array([[1,0],[0,-1]]))
qm.set_mixer_correction("mixer2", int(if_freq1), int(lo_freq),
                        tuple(m_pos.flatten())) #positive sideband, IF freq. 1
qm.set_mixer_correction("mixer2", int(if_freq2), int(lo_freq),
                        tuple(m_pos.flatten()))  # positive sideband, IF freq. 2
...
idx_p = np.abs(freqs-if_freq1).argmin()
idx_n = np.abs(freqs+if_freq1).argmin()
